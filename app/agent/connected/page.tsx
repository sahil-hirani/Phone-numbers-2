...
export default function ConnectedPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch('/api/agent/numbers?status=connected');
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Connected List</h1>
        <p className="text-gray-500 text-sm mt-1">
          <span className="font-semibold text-green-700">{numbers.length}</span> numbers marked as connected
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="bg-green-100 border-b-2 border-green-300 px-6 py-4">
          <h2 className="font-bold text-green-800 text-lg text-center tracking-wide">CONNECTED</h2>
        </div>
        ...
        {loading ? (
          ...
          <div className="p-12 text-center">
            <RiCheckboxCircleLine size={40} className="text-green-400 mx-auto mb-3" />
            <p className="text-gray-400 font-medium">No connected numbers yet.</p>
            <p className="text-gray-400 text-sm mt-1">Mark numbers as Connected in the FTD page.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse">
              ...
              <tbody>
                {numbers.map((n, idx) => (
                  <tr key={n._id} className="border-b border-gray-100 hover:bg-green-50/50 transition">
