...
import { RiDownloadLine, RiFilterLine, RiFlashlightLine, RiRefreshLine } from 'react-icons/ri';
...
export default function AdminActivePage() {
  ...
  function downloadExcel() {
    import('xlsx').then((XLSX) => {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Active Clients');
      XLSX.writeFile(wb, 'active_clients.xlsx');
    });
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <RiFlashlightLine className="text-emerald-500" size={26} />
            Active Clients
          </h1>
          <p className="text-gray-400 text-sm mt-0.5">Customers with activity in the last 7 days</p>
        </div>
      ...
      <div className="text-sm text-gray-500">
        Showing <span className="font-bold text-gray-800">{clients.length}</span> active clients
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-gray-400">No active clients found.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {clients.map((c, idx) => (
                  <tr key={c._id} className="hover:bg-emerald-50/40 transition">
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <span className="inline-flex items-center justify-center px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-700 font-semibold text-xs">
                        {c.activityCount}
                      </span>
                    </td>
                  </tr>
